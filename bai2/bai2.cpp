...
void DrawHead(float xPos, float yPos, float zPos)
{
	...
	glPushMatrix();
	glColor3f(0, 1, 0);
	glTranslatef(xPos - 1, yPos + 2, zPos - 1);
...
void Display()
{
	...
	if (Jump == 1) {
		glTranslatef(moveX, 8.0f + JumpY, moveZ);
		i++;
		if (i == 100) {
			Jump = 0;
			i = 0;
